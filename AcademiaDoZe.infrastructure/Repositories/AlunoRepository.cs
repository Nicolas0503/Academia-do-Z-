...
namespace AcademiaDoZe.infrastructure.Repositories
{
    public class AlunoRepository : BaseRepository<Aluno>, IAlunoRepository
    {
        ...
        public override async Task<Aluno> Adicionar(Aluno entity)
        {
            try
            {
                ...
                string query = _databaseType == DatabaseType.SqlServer
                    ? $"INSERT INTO {TableName} (cpf, telefone, nome, nascimento, email, logradouro_id, numero, complemento, senha, foto) OUTPUT INSERTED.id_aluno VALUES (@Cpf, @Telefone, @Nome, @Nascimento, @Email, @LogradouroId, @Numero, @Complemento, @Senha, @Foto);"
                    : $"INSERT INTO {TableName} (cpf, telefone, nome, nascimento, email, logradouro_id, numero, complemento, senha, foto) VALUES (@Cpf, @Telefone, @Nome, @Nascimento, @Email, @LogradouroId, @Numero, @Complemento, @Senha, @Foto); SELECT LAST_INSERT_ID();";
        ...
        public override async Task<Aluno> Atualizar(Aluno entity)
        {
            ...
            {
                await using var connection = await GetOpenConnectionAsync();
                string query = $"UPDATE {TableName} SET cpf = @Cpf, telefone = @Telefone, nome = @Nome, nascimento = @Nascimento, email = @Email, logradouro_id = @LogradouroId, numero = @Numero, complemento = @Complemento, senha = @Senha, foto = @Foto WHERE id_aluno = @Id";
                await using var command = DbProvider.CreateCommand(query, connection);
