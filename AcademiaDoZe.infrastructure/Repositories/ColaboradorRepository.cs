...
namespace AcademiaDoZe.infrastructure.Repositories
{
    public class ColaboradorRepository : BaseRepository<Colaborador>, IColaboradorRepository
    {
        ...
        public override async Task<Colaborador> Adicionar(Colaborador entity)
        {
            try
            {
                ...
                string query = _databaseType == DatabaseType.SqlServer
                    ? $"INSERT INTO {TableName} (cpf, telefone, nome, nascimento, email, logradouro_id, numero, complemento, senha, foto, admissao, tipo, vinculo) OUTPUT INSERTED.id_colaborador VALUES (@Cpf, @Telefone, @Nome, @Nascimento, @Email, @LogradouroId, @Numero, @Complemento, @Senha, @Foto, @Admissao, @Tipo, @Vinculo);"
                    : $"INSERT INTO {TableName} (cpf, telefone, nome, nascimento, email, logradouro_id, numero, complemento, senha, foto, admissao, tipo, vinculo) VALUES (@Cpf, @Telefone, @Nome, @Nascimento, @Email, @LogradouroId, @Numero, @Complemento, @Senha, @Foto, @Admissao, @Tipo, @Vinculo); SELECT LAST_INSERT_ID();";
        ...
        public override async Task<Colaborador> Atualizar(Colaborador entity)
        {
            ...
            {
                await using var connection = await GetOpenConnectionAsync();
                string query = $"UPDATE {TableName} SET cpf = @Cpf, telefone = @Telefone, nome = @Nome, nascimento = @Nascimento, email = @Email, logradouro_id = @LogradouroId, numero = @Numero, complemento = @Complemento, senha = @Senha, foto = @Foto, admissao = @Admissao, tipo = @Tipo, vinculo = @Vinculo WHERE id_colaborador = @Id";
                await using var command = DbProvider.CreateCommand(query, connection);
